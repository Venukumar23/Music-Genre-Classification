...
def runMetrics(name,cls):
    global accuracy, precision, recall, fscore, alg
    ...
    for i in range(len(accuracy)):
        output += '<tr><td><font size="" color="black">'+alg[i]+'</td><td><font size="" color="black">'+str(accuracy[i])+'</td><td><font size="" color="black">'+str(precision[i])+'</td><td><font size="" color="black">'+str(recall[i])+'</td><td><font size="" color="black">'+str(fscore[i])+'</td></tr>'
    conf_matrix = confusion_matrix(y_test, predict) 
    return output, conf_matrix

def TrainXGBoost(request):
    if request.method == 'GET':
        X_train = X[0:X.shape[0]-12]
        y_train = Y[0:Y.shape[0]-12]
        xgb_cls = XGBClassifier()
        xgb_cls.fit(X_train, y_train)
        output, conf_matrix = runMetrics("XGBoost Algorithm",xgb_cls)
        plt.figure(figsize =(6, 6)) 
        ax = sns.heatmap(conf_matrix, xticklabels = labels, yticklabels = labels, annot = True, cmap="viridis" ,fmt ="g");
        ax.set_ylim([0,10])
        plt.title("XGBoost Algorithm Confusion matrix") 
